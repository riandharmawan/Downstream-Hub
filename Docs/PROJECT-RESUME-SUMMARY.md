...
- **Deployment:** Two Alibaba Cloud servers — **172.28.92.56** (frontend), **172.28.92.57** (backend + DB).
...
| **Deployment guides** | [Docs/DEPLOYMENT-ALICLOUD.md](DEPLOYMENT-ALICLOUD.md) (host Node/Nginx/Postgres), [Docs/DEPLOYMENT-ALICLOUD-DOCKER.md](DEPLOYMENT-ALICLOUD-DOCKER.md) (Docker). Ports: frontend **3010** (or **3011** when using proxy), backend **4000**, Postgres **5434** on backend server (5432/5433 in use). |
| **Deploy package** | **deploy/** — docker-compose.frontend.yml, docker-compose.backend.yml, env examples, nginx configs, systemd unit. Build context set to `../Frontend` and `../Backend` so compose works when run from repo root with `-f deploy/...`. |
...
| **pgAdmin / DB access** | Staging Postgres is in Docker on 172.28.92.57, host port **5434**. Credentials in `/opt/downstream-hub/.env` (POSTGRES_USER, POSTGRES_PASSWORD, POSTGRES_DB). Security group must allow your IP to 5434 for pgAdmin from PC. |
...
- **Staging:** Frontend and backend are deployed and working.
  ...
  - **Postgres:** Docker on 172.28.92.57, host port **5434**.
...
|--------|------|-------------------|--------|
| **172.28.92.56** | Frontend + Nginx proxy | 3010 (frontend container), **3011** (Nginx — user entry point) | Clone in /opt/downstream-hub; Docker frontend + host Nginx. |
| **172.28.92.57** | Backend + Postgres | 4000 (API), 5434 (Postgres host) | Clone in /opt/downstream-hub; docker-compose.backend.yml. API_PUBLIC_URL must be http://172.28.92.56:3011. |
