/**
 * One-time forgot-password verification tokens (hash stored, never plaintext).
 */
async function insert(db, { userId, tokenHash, expiresAt, requestIp }) {
  const { rows } = await db.query(
    `INSERT INTO forgot_password_verify_tokens (user_id, token_hash, expires_at, request_ip)
     VALUES ($1, $2, $3, $4)
     RETURNING id`,
...
async function findActiveByHash(db, tokenHash) {
  const { rows } = await db.query(
    `SELECT id, user_id, expires_at, used_at
     FROM forgot_password_verify_tokens
...
async function markUsed(db, id) {
  await db.query(
    `UPDATE forgot_password_verify_tokens
     SET used_at = now()
