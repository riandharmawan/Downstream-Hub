...
export default function ForgotPasswordVerify() {
  ...
  useEffect(() => {
    async function verify() {
      ...
      try {
        const info = await apiRequest(`/api/auth/forgot-password-verify/token-info?token=${encodeURIComponent(token)}`);
        if (!info.valid) {
        ...
        setMessage('Verifying your reset request...');
        const result = await apiRequest('/api/auth/forgot-password-verify', {
          method: 'POST',
          body: JSON.stringify({ token }),
        });
        if (!result.reset_token) {
          throw { error: 'Verification completed, but reset token was missing.' };
        }
        navigate(`/reset-password?token=${encodeURIComponent(result.reset_token)}`, {
          replace: true,
          state: { message: 'Email verification complete. You can now set a new password.' },
        });
      } catch (err) {
        setError(err.error || 'Failed to verify reset request.');
        setMessage('');
        setLoading(false);
      }
    }
  ...
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        <h1 style={styles.title}>Verify password reset</h1>
