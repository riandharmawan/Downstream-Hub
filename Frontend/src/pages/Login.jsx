...
import { getSsoLoginParams, resumeAfterLogin, ssoLoginBodyExtras } from '../lib/ssoReturnTo';
...
export default function Login() {
  ...
  const { returnTo, clientId } = useMemo(
    () => getSsoLoginParams(location.search),
    [location.search]
  );
  const ssoExtras = useMemo(
    () => ssoLoginBodyExtras(returnTo, clientId),
    [returnTo, clientId]
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await apiRequest(`/api/sso/login-context?client_id=${encodeURIComponent(clientId)}`);
        if (!ignore) setAppContext(data);
      } catch {
        if (!ignore) setAppContext(null);
      }
    })();
    return () => {
      ignore = true;
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true);
    try {
      const result = await login(email, password, ssoExtras);
  ...
  async function handleResendMagicLink() {
    ...
    setError('');
    try {
      const result = await resendMagicLink(email, password, magicLinkPending?.pending_id, ssoExtras);
      setMagicLinkPending(result);
    } catch (err) {
  ...
  const appIconSrc = appContext?.icon_url ? resolveIconSrc(appContext.icon_url) : null;
      ...
      <div style={styles.card}>
        ...
        {appContext?.app_name ? (
          <div style={styles.appBanner}>
            {appIconSrc && (
              <img src={appIconSrc} alt="" style={styles.appIcon} />
            )}
            <p style={styles.appBannerText}>
              Sign in to continue to <strong>{appContext.app_name}</strong>
            </p>
          </div>
        ) : (
...
const styles = {
  ...
  appBanner: { display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 'var(--space-4)', padding: 'var(--space-2) var(--space-3)', background: 'var(--color-bg-light)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--color-border-light)' },
  appIcon: { width: 32, height: 32, borderRadius: 'var(--radius-sm)', objectFit: 'contain', flexShrink: 0 },
  appBannerText: { margin: 0, color: 'var(--color-text-steel)', fontSize: 'var(--text-small)', lineHeight: 1.4 },
